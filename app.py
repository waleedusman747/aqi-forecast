...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Built by Waleed | FYP: AIRecruitX-style serverless ML project")
...
with col2:
    ...
    m1.metric("PM2.5", f"{latest['pm25']:.0f}")
    m2.metric("Temperature", f"{latest['temperature']:.0f}°C")
    m3.metric("Humidity", f"{latest['humidity']:.0f}%")
    m4.metric("Wind", f"{latest['wind']:.0f} km/h")
